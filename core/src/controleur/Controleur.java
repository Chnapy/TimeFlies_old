 ...
 *
 */
public class Controleur implements Observer, Tourable {
	...
	public Controleur(Map m, Joueur[] joueurs) {
		map = m;
		tabJoueurs = joueurs;
...
//					System.out.println(oriAttaque);
	...
	public void modeSort(int index) {
		...
		if (sort.getCooldownActu() == 0) {
			...
			chatCombatPrint("Vous ne pouvez pas lancer le sort " + sort.getNom() + ". Le cooldown est de " + sort.getCooldownActu() + " tours.", ChatTextType.IMPORTANT);
		}
	}
